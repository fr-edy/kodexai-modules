...
DATABASE_HOST: str = "https://www.ecb.europa.eu/foedb/dbs/foedb"
...
def load_publications(publications_url: str, updates_type: str) -> List[Dict]:
...
def load_publications_lazy_loaded(
    publications_url: str, updates_type: str
) -> List[Dict]:
    """Loads the last 10 publications from the ECB website and extracts the PDF links."""
    return _load_last_publications(publications_url, updates_type)
...
def _get_url(
    ...
    db_root = f"{DATABASE_HOST}/{DATABASE_NAME}/"

    if request == "versions":
        return f"{db_root}versions.json"

    versioned_root = (
        f"{db_root}{loaded_db['database_version']}/{loaded_db['database_hash']}/"
    )
...
def _get_chunk(chunk_id: int, loaded_db: dict) -> list:
    """
...
def load_publications_from_db(
    update_type: RegUpdateTypes, amount_to_fetch: int = 10
) -> List[Dict]:
...
def _load_publications_db(
    ...
    return [
        _parse_db_publications_as_publication(update_type, r) for r in matching_releases
    ]


def _parse_db_publications_as_publication(
    update_type: RegUpdateTypes, release: Dict
) -> Dict:
...
def _fetch_foe_db_data(
    ...
    try:
        ...
        versions = _fetch_json(_get_url("versions", loaded_db))
        loaded_db["database_version"] = versions[0]["version"]
        loaded_db["database_hash"] = versions[0]["hash"]
        loaded_db["metadata"] = _fetch_json(_get_url("metadata", loaded_db))
        loaded_db["indexes"] = {}
...
def _parse_related_publication(raw_data: str) -> Dict:
    ...
    try:
        ...
        log.error(f"Error parsing related publication: {e}")
        return {}
