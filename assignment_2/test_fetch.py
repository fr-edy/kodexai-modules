...
class FoeDBFetcher:
        ...
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as session:
            async with session.get(url) as response:
                if response.status == 404:
                    # Handle 404 by refreshing metadata and retrying
                    await self.refresh_db_metadata()
                    async with session.get(url) as retry_response:
                        if not retry_response.ok:
                            raise Exception(f"Failed to fetch {url}: {retry_response.status}")
                        return await retry_response.json()
                elif not response.ok:
                    raise Exception(f"Failed to fetch {url}: {response.status}")
                return await response.json()

    def get_url(self, request: str, key: Optional[str] = None, 
    ...
    def build_data_from_chunk(self, sort_id: int, chunk: List) -> Dict:
        ...
        for i, field_name in enumerate(header):
            ...
            if 'loaded_maps' in self.loaded_db and field_name in self.loaded_db['loaded_maps']:
                result[field_name] = self.loaded_db['loaded_maps'][field_name]['index'][data[i]]
            else:
    ...
    async def load_db(self):
        ...
        self.loaded_db['database_version'] = self.config.database_version or versions[0]['version']
